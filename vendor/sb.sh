#!/bin/bash
# ===================================================================
# ccAzy fork 增强: Argo 临时隧道支持附加参数文件 /etc/s-box/argo-extra.conf
# 该文件可选,不存在时行为与上游完全一致;存在时其内容(如
#   --protocol quic --edge-ip-version 4
# )会附加到 cloudflared 启动命令,用于按 VPS 网络环境调优隧道稳定性。
# 修改后无需重启: 下次 sb 菜单操作/重启隧道时自动生效。
# ===================================================================
