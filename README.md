...
- WARP 域名分流
- **IPv4 锁定**：入口 IP 走 SB 15-1 切换（订阅 server: 用 IPv4），出口 `route/outbounds/dns` 仅 `--force` 时原子 `jq` 改 `ipv4_only`（`sb-yg` 重建回退 `prefer_ipv6` 自动纠回）、`gai.conf` 单行幂等、`sing-box 1.12+ legacy` 兼容（`ENABLE_DEPRECATED...`）
...
- **IPv4 锁定**：`gai.conf 单行 / legacy env / 出口 route/outbounds/dns ipv4_only` + **订阅入口 IP 已切 IPv4**（JP/HK 订阅回退根因）
- sing-box 是否安装和运行
- BBRv3 是否存在
- fq 和网卡优化状态
- 持久化网络调优服务
...
- 验证状态（**2026-09-23 更新，注意时效**）：
  - **Ubuntu 22.04：已实测** —— 第 1 步（内核安装 / 网络调优 / GRUB 默认项 / 自动重启）在 QQ、QQ2 两台真机多次跑通（含慢速链路断点续传）。
  - **Debian 12：待复验** —— 下面那句“5 个直连协议 + Argo 端到端连通”是在 **Debian 12 / HK** 上做的，属**本轮改动之前**的验证；
    本轮改动（下载续传、GRUB 路径定位、conntrack 开机加载、SSH 端口探测、lib 自举刷新）**只在 Ubuntu 上验过**。
