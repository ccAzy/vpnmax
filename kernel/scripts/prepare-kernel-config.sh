...
case "$arch" in
arm64)
  cp "$GITHUB_WORKSPACE/kernel/arm64.config" .config
  ;;
x86_64)
  cp "$GITHUB_WORKSPACE/kernel/x86-64.config" .config
  ;;
*)
...
mkdir -p "$GITHUB_WORKSPACE/kernel/build-configs"
cp .config "$GITHUB_WORKSPACE/kernel/build-configs/${arch}.config"
cp .config "$GITHUB_WORKSPACE/kernel/build-configs/${arch}-${KERNEL_VERSION}.config"
