...
def read_kernel_config(kernel_release: str) -> str | None:
    boot_cfg = f"/boot/config-{kernel_release}"
    if os.path.exists(boot_cfg):
        with open(boot_cfg, encoding="utf-8", errors="ignore") as f:
            return f.read()
...
def parse_aead_config(config_text: str | None) -> str:
    if not config_text:
        return "未知（未找到内核配置）"
...
def parse_tristate_symbol(config_text: str | None, symbol: str) -> str:
    if not config_text:
        return "未知（未找到内核配置）"
...
def is_module_loaded(module_name: str) -> bool:
    try:
        with open("/proc/modules", encoding="utf-8", errors="ignore") as f:
            for line in f:
