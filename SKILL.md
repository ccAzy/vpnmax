...
3. **内核校验和＝尽力而为（不阻断）**：上游 release 提供 `SHA256SUMS` 就比对，**没有或对不上只告警**，不影响安装。上游（`byJoey/Actions-bbr-v3` → `ccAzy` fork → 本仓 release）都不产出校验和，所以不把它当门禁——2026-09-23 拍板：默认信任上游，不为其维护哈希资产。
...
8. **dry-run + 备份**：两部署脚本 + cleanup 均支持 `--dry-run`；cleanup 前自动备份规则到 `/var/backups/vpnmax/`。
9. **conntrack 协同调优**：`nf_conntrack_max` 按内存分级，并在模块支持时同步 hashsize，避免只增大表上限而放大桶冲突。
10. **iptables 持久化双保险**：端口跳跃/防探测规则除 netfilter-persistent 外，额外落盘 `/etc/iptables/rules.v4|v6` 并注册 `vpnmax-netfilter-restore.service`（network-pre.target 前恢复），防重启后 `VPNMAX_*` 链丢失。
...
>
> 校验和策略：上游 release 有 `SHA256SUMS` 就比对，**没有就跳过（只告警）**，不阻断安装。
...
3. 配置 Hysteria2 端口跳跃（40000-42000）+ Tuic 端口跳跃（43000-45000）→ 独立链 `VPNMAX_PORTHOP`
4. 启动 Argo 临时隧道（Cloudflare CDN 隐藏 IP）+ **三级保活 v3**（flock 互斥/进程+HTTP双检/僵死重连/翻动冷却）
5. 应用安全加固（独立链 `VPNMAX_ANTIPROBE` + 网络感知 sysctl + **iptables 双保险持久化**）
6. WARP + 域名分流（可选，失败不影响核心）
7. 日志轮转（`/etc/logrotate.d/vpnmax`）
...
> cleanup 只删 vpnmax 自己的 `VPNMAX_ANTIPROBE`/`VPNMAX_PORTHOP`/`VPNMAX_RSS` 链，并清理 `vpnmax-net-tuning.service` 与其调优脚本；自动备份原规则到 `/var/backups/vpnmax/`。crontab/grep 用 `|| true` 防 set -e 静默退出。
...
- `/var/log/vpnmax-optimize-manifest.log`（内核来源/版本/SHA256）
- `/var/log/vpnmax-singbox-manifest.log`（sb.sh commit/SHA256、部署结果、VMESS_LOCK 值）
...
- 独立防火墙链名沿用 `VPNMAX_*` 前缀是为了兼容清理旧 ACVPN 安装，属有意保留，勿改。
